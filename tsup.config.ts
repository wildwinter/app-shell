...
export default defineConfig({
  entry: [
    ...
    "src/updater-view.ts", "src/welcome.ts", "src/link-status.ts", "src/property-list.ts", "src/util.ts", "src/keys.ts", "src/problems.ts",
    ...
    "src/updater.css", "src/welcome.css", "src/link-status.css", "src/keys.css",
  ],
  format: ["esm", "cjs"],
  // Declarations only for the TS entries (the dts compiler rejects css roots).
  dts: { entry: ["src/index.ts", "src/menu.ts", "src/tool-window.ts", "src/job.ts", "src/vc-status.ts", "src/app-store.ts", "src/updater.ts", "src/session.ts", "src/dialog.ts", "src/floating.ts", "src/context-menu.ts", "src/updater-view.ts", "src/welcome.ts", "src/link-status.ts", "src/property-list.ts", "src/util.ts", "src/keys.ts", "src/problems.ts"] },
  clean: true,
  sourcemap: true,
  treeshake: true,
